...
string CMd5Util::GetMD5(const string &buf)
{
	MD5_CTX context;
	unsigned char source[16] = {0};
	MD5Init(&context);
	MD5Update(&context, (unsigned char *)buf.data(), buf.size());
	MD5Final((unsigned char *)source, &context);

	std::string strMd5;
	for (int i=0; i<16; i++)
	{
		strMd5.push_back(("0123456789abcdef")[source[i] >> 4]);
		strMd5.push_back(("0123456789abcdef")[source[i] & 0xf]);
	}

#if !defined(AFX_FTPSERVERUTIL_H__1E8C6654_CFCA_4759_A758_11A46F880254__INCLUDED_)
#define AFX_FTPSERVERUTIL_H__1E8C6654_CFCA_4759_A758_11A46F880254__INCLUDED_
...
namespace sdk
{
	...
	class AFX_EXT_CLASS CFtpServerUtil
	{
	...
	public:
		...
		public:
			...
			/* Create a new User. Don't create a user named "anonymous", call AllowAnonymous();.
				...
				Returns:
					...
					-on error: NULL.
			*/
			struct CFtpServerUtil::UserNode *AddUser( const char *szName, const char *szPasswd, const char* szStartDir );
			...
			/* Set the Privilegies of a User.
				...
				Returns:
					...
					-on error: false.
			*/
			bool SetUserPriv( struct CFtpServerUtil::UserNode *User, unsigned char ucPriv );
			...
			/* Delete a User, and by the way all the Clients connected to this User.
				...
				Returns:
					...
					-on error: false.
			*/
			bool DelUser( struct CFtpServerUtil::UserNode * User );
			...
			*/
			struct CFtpServerUtil::UserNode *GetUserListHead( void ) {
			...
			*/
			struct CFtpServerUtil::UserNode *GetUserListLast( void ) {
			...
			*/
			struct CFtpServerUtil::UserNode *GetNextUser( const struct CFtpServerUtil::UserNode *User ) {
				if( User )
			...
			*/
			struct CFtpServerUtil::UserNode *GetPreviousUser( const struct CFtpServerUtil::UserNode *User ) {
				if( User )
			...
			/* Set the maximum number of Clients which can be connected to the User at a time.
				...
				Returns:
					...
					-on error: false.
			*/
			bool SetUserMaxClient( struct CFtpServerUtil::UserNode *User, int iMaxClient );
			...
			/* Get the maximum number of Clients which can be connected to the User at a time.
				...
				Returns:
					...
					-on error: -2.
			*/
			unsigned int GetUserMaxClient( const struct CFtpServerUtil::UserNode *User ) {
				if( User )
			...
			/* Get a User's privilegies 
				...
				Returns:
					...
					-on error: 0.
			*/
			unsigned char GetUserPriv( const struct CFtpServerUtil::UserNode *User ) {
				if( User ) {
					return User->ucPriv;
				} else return 0;
			}
			...
			/* Get a pointer to a User's Name.
				...
				Returns:
					...
					-on error: NULL.
			*/
			const char* GetUserLogin( const struct CFtpServerUtil::UserNode *User ) {
				if( User ) {
					return User->szName;
				} else return NULL;
			}
			...
			/* Get a pointer to a User's Password.
				...
				Returns:
					...
					-on error: NULL.
			*/
			const char* GetUserPasswd( const struct CFtpServerUtil::UserNode *User ) {
				if( User ) {
			...
			/* Get a pointer to a User's Start Directory.
				...
				Returns:
					...
					-on error: NULL.
			*/
			const char* GetUserStartDir( const struct CFtpServerUtil::UserNode *User ) {
				if( User ) {
				...
				/* Set the supported Extra-Commands of a User.
					...
					Returns:
						...
						-on error: false.
				*/
				bool SetUserExtraCmd( struct CFtpServerUtil::UserNode *User, unsigned char dExtraCmd );
				...
				/* Get the supported Extra-Commands of a User.
					...
					Returns:
						...
						-on error: 0.
				*/
				unsigned char GetUserExtraCmd( const struct CFtpServerUtil::UserNode *User ) {
					if( User ) {
			...
			*/
			...
			/* Get the number of Clients currently connected using the specified User.
				...
				Returns:
					...
					-on error: -1.
			*/
			int GetNbClientUsingUser( struct CFtpServerUtil::UserNode *User ) {
				if( User ) {
		...
		private:
			...
			struct ClientNode
			{
				...
				class CFtpServerUtil *CFtpServerEx;
			...
			/* Add a new Client.
				Returns:
					...
					-on error: NULL.
			*/
			struct CFtpServerUtil::ClientNode *AddClient( CFtpServerUtil *CFtpServerEx, SOCKET Sock, struct sockaddr_in *Sin );
			...
			/* Delete a Client.
				Returns:
					...
					-on error: false.
			*/
			bool DelClientEx( struct CFtpServerUtil::ClientNode *Client );
			...
			*/
			...
			struct CFtpServerUtil::ClientNode *ClientListHead;
			struct CFtpServerUtil::ClientNode *ClientListLast;
			...
			*/
			bool SendReply( struct CFtpServerUtil::ClientNode *Client, const char *pszReply );
			bool SendReply2( struct CFtpServerUtil::ClientNode *Client, const char *pszList, ... );
			...
			*/
			...
			/* Process the LIST, NLST Commands.
				Returns:
					...
					-on error: false.
			*/
			bool LIST_Command( struct CFtpServerUtil::ClientNode *Client, char *pszCmdArg, const char *pszCmd );
			...
			/* Process the STOR, STOU, APPE Commands.
				Returns:
					...
					-on error: false.
			*/
			bool STOR_Command( struct CFtpServerUtil::ClientNode *Client, char *pszCmdArg, const char *pszCmd );
			...
			/* Open the Data Canal in order to transmit data.
				...
				-on error: false.
			*/
			bool OpenDataConnection(struct CFtpServerUtil::ClientNode *Client);
			...
			bool bIsAccepting;
				...
				static unsigned __stdcall CFtpServerUtil::StartAcceptingEx( void *pvParam );
			#else

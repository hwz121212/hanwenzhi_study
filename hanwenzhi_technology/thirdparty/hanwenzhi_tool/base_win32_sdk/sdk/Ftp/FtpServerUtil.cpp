...
using namespace sdk;
	...
	#define O_TRUNC		_O_TRUNC
		...
		#define _findfirst1	_findfirsti64
		#define _findnext1	_findnexti64
	#endif
...
CFtpServerUtil::CFtpServerUtil(void)
{
	int i=0;
...
bool CFtpServerUtil::AllowAnonymous( bool bDoAllow, const char *szStartPath )
{
	if( bDoAllow ) {
		if( this->IsAnonymousAllowed() == false ) {
			if( (this->AnonymousUser = this->AddUserEx( "anonymous", NULL, szStartPath )) == NULL )
				return false;
			this->SetUserPriv( this->AnonymousUser, CFtpServerUtil::READFILE | CFtpServerUtil::LIST );
		}
...
bool CFtpServerUtil::SetDataPortRange( unsigned short int usDataPortStart, unsigned int iNumber )
{
 ...
 * NETWORK
 ***************************************/
bool CFtpServerUtil::StartListening( unsigned long ulAddr, unsigned short int usPort )
{
...
bool CFtpServerUtil::StopListening( void )
{
	if( this->IsListening() ) {
		this->CloseSocket( this->ListeningSock );
		this->ListeningSock = INVALID_SOCKET;
...
bool CFtpServerUtil::StartAccepting( void )
{
...
#ifdef WIN32
	unsigned CFtpServerUtil::StartAcceptingEx( void *pvParam )
#else
	void* CFtpServerUtil::StartAcceptingEx( void *pvParam )
#endif
{
	class CFtpServerUtil *CFtpServerEx = ( class CFtpServerUtil* ) pvParam;
	...
	while ( 1 ) {
		...
		if( TempSock != INVALID_SOCKET ) {
			struct CFtpServerUtil::ClientNode *NewClient = 
 ...
 ***************************************/
#ifdef WIN32
	unsigned CFtpServerUtil::ClientShell( void *pvParam )
#else
	void* CFtpServerUtil::ClientShell( void *pvParam )
#endif
{
	struct CFtpServerUtil::ClientNode *Client=(struct CFtpServerUtil::ClientNode*)pvParam;
	class CFtpServerUtil *CFtpServerEx=Client->CFtpServerEx;
		...
		if( !strcmp( pszCmd, "QUIT" ) ) {
			...
			if( Client->bIsAnonymous ) {
				...
				if( pszCmdArg ) {
					...
					{
						if( Client->User->iMaxClient == CFtpServerUtil::Unlimited
			...
			if( pszCmdArg ) {
				...
				if( !strncasecmp( pszCmdArg, "EXEC ", 5 ) ) {
					if( (ucExtraCmd & CFtpServerUtil::ExtraCmd_EXEC) == CFtpServerUtil::ExtraCmd_EXEC ) {
			...
			if( pszCmdArg && Client->eStatus == CFtpServerUtil::Status_Waiting ) {
			...
			if( !(Client->User->ucPriv & CFtpServerUtil::READFILE) ) {
				CFtpServerEx->SendReply( Client, "550 Permission denied.\r\n");
				continue;
			}
			if( pszCmdArg ) {

				if( Client->eStatus != CFtpServerUtil::Status_Waiting )	{
					CFtpServerEx->SendReply( Client, "425 You're already connected.\r\n");
					continue;
				}
				if( Client->eDataMode == CFtpServerUtil::Mode_None  || !CFtpServerEx->OpenDataConnection(Client) ) {
				...
				if( pszPath && stat( pszPath, &st ) == 0 ) {
					Client->eStatus = CFtpServerUtil::Status_Downloading;
					...
					CFtpServerEx->SendReply( Client, "150 Opening data connection.\r\n");
						...
						Client->eStatus = CFtpServerUtil::Status_Waiting;
					}
			...
			if( !(Client->User->ucPriv & CFtpServerUtil::WRITEFILE) ) {
				CFtpServerEx->SendReply( Client, "550 Permission denied.\r\n");
				continue;
			}
			if( pszCmdArg || (!strcmp( pszCmd, "STOU" )) ) {
				...
				if( pszPath ) {
					Client->eStatus = CFtpServerUtil::Status_Uploading;
					Client->CurrentTransfer.Client = Client;
			...
			if( !(Client->User->ucPriv & CFtpServerUtil::DELETEFILE) ) {
				CFtpServerEx->SendReply( Client, "550 Permission denied.\r\n");
				continue;
			}
			if( pszCmdArg ) {
			...
			if( !(Client->User->ucPriv & CFtpServerUtil::CREATEDIR) ) {
				CFtpServerEx->SendReply( Client, "550 Permission denied.\r\n");
				continue;
			}
			if( pszCmdArg ) {
			...
			if( !(Client->User->ucPriv & CFtpServerUtil::DELETEDIR) ) {
				CFtpServerEx->SendReply( Client, "550 Permission denied.\r\n");
				continue;
			}
			if( pszCmdArg ) {
	...
	if( Client->eStatus != CFtpServerUtil::Status_Uploading
		&& Client->eStatus != CFtpServerUtil::Status_Downloading )
	{
...
bool CFtpServerUtil::SendReply( struct CFtpServerUtil::ClientNode *Client, const char *pszReply )
{
...
bool CFtpServerUtil::SendReply2( struct CFtpServerUtil::ClientNode *Client, const char *pszList, ... )
{
...
bool CFtpServerUtil::LIST_Command( struct CFtpServerUtil::ClientNode *Client, char *pszCmdArg, const char *pszCmd )
{
	if( !Client )
		return false;
	class CFtpServerUtil *CFtpServerEx = Client->CFtpServerEx;

	if( !(Client->User->ucPriv & CFtpServerUtil::LIST) ) {
		CFtpServerEx->SendReply( Client, "550 Permission denied.\r\n");
		return false;
	}
	if( Client->eStatus != Status_Waiting )	{
...
bool CFtpServerUtil::OpenDataConnection( struct CFtpServerUtil::ClientNode *Client )
{
...
#ifdef WIN32
	unsigned CFtpServerUtil::StoreThread( void *pvParam )
#else
	void* CFtpServerUtil::StoreThread( void *pvParam )
#endif
{
	struct CFtpServerUtil::ClientNode *Client = (struct CFtpServerUtil::ClientNode*) pvParam;
	...
	if( Client ) {
		...
		#endif
		memset( &Client->CurrentTransfer, 0x0, sizeof( Client->CurrentTransfer ) );
		Client->eDataMode = CFtpServerUtil::Mode_None;
		if( Client->IsCtrlCanalOpen == false ) {
			CFtpServerEx->DelClientEx( Client );
		} else Client->eStatus = CFtpServerUtil::Status_Waiting;
	}
	#ifdef WIN32
		_endthreadex( 0 );
		return 0;
...
#ifdef WIN32
	unsigned CFtpServerUtil::RetrieveThread( void *pvParam )
#else
	void* CFtpServerUtil::RetrieveThread( void *pvParam )
#endif
{
	struct CFtpServerUtil::ClientNode *Client = (struct CFtpServerUtil::ClientNode*) pvParam;
...
struct CFtpServerUtil::LIST_FindFileInfo *CFtpServerUtil::LIST_FindFirstFile( char *szPath )
{
	if( szPath ) {

		struct LIST_FindFileInfo *fi = new struct CFtpServerUtil::LIST_FindFileInfo;
		memset( fi, 0x0, sizeof (struct CFtpServerUtil::LIST_FindFileInfo));
...
bool CFtpServerUtil::LIST_FindNextFile( struct CFtpServerUtil::LIST_FindFileInfo *fi )
{
	if( fi ) {
			...
			bool i = false;
			if( fi->hFile != -1L ) {
				if( _findnext1( fi->hFile, &fi->c_file ) == 0 )
					i = true;
			} else {
				fi->hFile = _findfirst1( fi->pszTempPath, &fi->c_file );
				if( fi->hFile != -1 )
					i = true;
			}
...
void CFtpServerUtil::LIST_FindClose( struct CFtpServerUtil::LIST_FindFileInfo *fi )
{
	if( fi ) {
		#ifdef WIN32
			if( fi->pszTempPath )
...
char* CFtpServerUtil::BuildPath( const char *szStartPath, const char *szCurrentPath, char *szAskedPath )
{
	if( szStartPath && szCurrentPath ) {
		...
		if( !szAskedPath || szAskedPath[0] == '\0'  ) {
			...
			CFtpServerUtil::SimplifyPath( szAskedPath );
...
bool CFtpServerUtil::SimplifyPath( char *pszPath )
{
...
struct CFtpServerUtil::UserNode *CFtpServerUtil::AddUser( const char *szName, const char *szPasswd, const char *szStartDir)
{
...
struct CFtpServerUtil::UserNode *CFtpServerUtil::AddUserEx( const char *szName, const char *szPasswd, const char *szStartDir)
{
		...
		&& ( (szPasswd && szPasswd[0] != '\0' ) || !strcasecmp( "anonymous", szName ) ) )
	{
		...
		struct CFtpServerUtil::UserNode *NewUser = new struct CFtpServerUtil::UserNode;
		memset( NewUser, 0x0, sizeof( struct CFtpServerUtil::UserNode ) );
...
#ifdef CFTPSERVER_USE_EXTRACMD
bool CFtpServerUtil::SetUserExtraCmd( struct CFtpServerUtil::UserNode *User, unsigned char ucExtraCmd )
{
...
bool CFtpServerUtil::SetUserMaxClient( struct CFtpServerUtil::UserNode *User, int iMaxClient )
{
...
bool CFtpServerUtil::SetUserPriv( struct CFtpServerUtil::UserNode *User, unsigned char ucPriv )
{
...
bool CFtpServerUtil::DelUser(struct CFtpServerUtil::UserNode *User)
{
	if( User != NULL ) {
		...
		if( User->iNbClient > 0 ) {
			struct CFtpServerUtil::ClientNode *Client = this->ClientListHead;
			struct CFtpServerUtil::ClientNode *NextClient = NULL;
...
struct CFtpServerUtil::UserNode * CFtpServerUtil::SearchUserFromName( const char *szName )
{
	if( szName ) {
		...
		struct CFtpServerUtil::UserNode *SearchUser = UserListHead;
		struct CFtpServerUtil::UserNode *ReturnedUser = NULL;
		while( SearchUser ) {
...
void CFtpServerUtil::DelAllUser( void )
{
	struct CFtpServerUtil::UserNode *User = this->UserListHead;
	struct CFtpServerUtil::UserNode *UserNext = NULL;
	while( User ) {
...
struct CFtpServerUtil::ClientNode *CFtpServerUtil::AddClient( CFtpServerUtil *CFtpServerEx, SOCKET Sock, struct sockaddr_in *Sin )
{
	if( Sock != INVALID_SOCKET && &Sin && CFtpServerEx ) {

		struct CFtpServerUtil::ClientNode *NewClient = new struct CFtpServerUtil::ClientNode;
		memset( NewClient, 0x0, sizeof(struct CFtpServerUtil::ClientNode) );
...
bool CFtpServerUtil::DelClientEx( struct CFtpServerUtil::ClientNode * Client)
{
	if( Client != NULL ) {
...
void CFtpServerUtil::DelAllClient( void )
{
